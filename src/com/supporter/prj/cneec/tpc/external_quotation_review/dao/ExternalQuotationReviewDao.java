package com.supporter.prj.cneec.tpc.external_quotation_review.dao;
...
/**
 * @Title: ExternalQuotationReviewDao
 * @Description: DAO类
 * @author: yanweichao
 * @date: 2017-09-25
 * @version: V1.0
 */
@Repository
public class ExternalQuotationReviewDao extends MainDaoSupport<ExternalQuotationReview, String> {

	/**
	 * 分页查询
	 */
	public List<ExternalQuotationReview> findPage(JqGrid jqGrid, ExternalQuotationReview externalQuotationReview, String authFilter) {
		if (externalQuotationReview != null) {
			// 列表页面搜索输入框可查询字段
			String keyword = externalQuotationReview.getKeyword();
			...
			/* 以下是更多条件中选择项 */
			// 状态过滤
			if (externalQuotationReview.getSwfStatus() != null) {
				jqGrid.addHqlFilter(" swfStatus = ? ", externalQuotationReview.getSwfStatus());
			}
			// 根据创建时间倒序排列
	...
	public List<ExternalQuotationReview> getList(Map<String, Object> parameters, String authFilter) throws ParseException {
		String hql = getHql(parameters, authFilter);
		return this.retrieve(hql, parameters);
	}

	public ListPage<ExternalQuotationReview> getListPage(ListPage<ExternalQuotationReview> listPage, Map<String, Object> parameters, String authFilter) throws ParseException {
		String hql = getHql(parameters, authFilter);
		return this.retrievePage(listPage, hql, parameters);
	}

	@SuppressWarnings("unchecked")
	public List<ExternalQuotationReview> getExternalQuotationReviewList(Map<String, Object> parameters) throws ParseException {
	...
	private String getHql(Map<String, Object> parameters, String authFilter) throws ParseException {
		...
		hql.append("from " + ExternalQuotationReview.class.getName() + " t where 1=1");
		// 状态(默认为完成)
		Integer swfStatus = (Integer) parameters.get("swfStatus");
		if (swfStatus == null || swfStatus != 0) {
			swfStatus = ExternalQuotationReview.COMPLETED;
		}
