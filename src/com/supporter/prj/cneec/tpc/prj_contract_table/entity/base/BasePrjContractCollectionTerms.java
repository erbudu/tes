 ...
 */
@MappedSuperclass
public class BasePrjContractCollectionTerms implements Serializable {
	...
	private double receiveAmount;// 合同应收款/付款金额
	private double receiveProportion;
	private double receiveRmbAmount;
	private String receiveDate;
	private String collectionTerms;
	private double onwayAmount;// 在途金额（按原币金额算）
	private double realReceiveAmount;// 实际收款/付款金额（按原币金额算）

	/**
	 * 无参构造函数.
	 */
	public BasePrjContractCollectionTerms() {
	}
	 ...
	 * @param termsId
	 */
	public BasePrjContractCollectionTerms(String termsId) {
	...
	/**
	 * 收款/付款在途金额
	 *  合同上线后为0，在合同履行销售合同收款提交后写入，采购合同付款提交后写入
	 * @return
	 */
	...
	/**
	 * 实际收款/付款金额
	 *  合同上线后为0，在合同履行销售合同收款完成后写入，采购合同付款完成后写入
	 * @return
	 */
	@Column(name = "REAL_RECEIVE_AMOUNT", nullable = true, precision = 10, scale = 3)
