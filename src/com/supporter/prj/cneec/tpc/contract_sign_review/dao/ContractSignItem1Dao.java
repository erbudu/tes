 ...
 */
@Repository
public class ContractSignItem1Dao extends MainDaoSupport<ContractSignItem1, String> {

	/**
	 * 分页查询
	 */
	public List<ContractSignItem1> findPage(JqGrid jqGrid, Map<String, Object> parameters) {
	...
	public List<ContractSignItem1> findList(Map<String, Object> parameters) {
		StringBuffer hql = new StringBuffer();
		hql.append("from " + ContractSignItem1.class.getName());
		String filter = "";
	...
	public void removePurchaseValue(String inforId) {
		String hql = "update " + ContractSignItem1.class.getName() + " set purchaseReviewId = null, purchaseContractId = null where purchaseContractId = ?";
	...
	@SuppressWarnings("unchecked")
	public List<ContractSignItem1> getList(Map<String, Object> parameters, String authFilter) throws ParseException {
		String hql = getHql(parameters, authFilter);
		return this.retrieve(hql, parameters);
	}

	public ListPage<ContractSignItem1> getListPage(ListPage<ContractSignItem1> listPage, Map<String, Object> parameters, String authFilter) throws ParseException {
		String hql = getHql(parameters, authFilter);
	...
	private String getHql(Map<String, Object> parameters, String authFilter) throws ParseException {
		StringBuffer hql = new StringBuffer();
		hql.append("from " + ContractSignItem1.class.getName() + " t where 1 = 1");
		// 过滤项目，必要条件
		if (parameters.containsKey("projectId")) {
			hql.append(" and t.saleReviewId in (select s.signId from " + ContractSignDeptReview.class.getName() + " s where s.projectId = :projectId)");
		} else {
			hql.append(" and 1 <> 1");
		}
		// 选择过滤条件
		if (parameters.containsKey("selectTo")) {
			...
			if (selectTo == ContractSignReviewUtil.INFOR_TYPE_CONTRACT) {
				...
				hql.append(" or ( t.leaf = 'F' and t.itemId in (select t1.parentItemId from " + ContractSignItem1.class.getName() + " t1 where t1.purchaseContractId is null) )");
				hql.append(" )");
				if (parameters.containsKey("initialReviewType")) {
					...
					if (initialReviewType == ContractSignReviewUtil.REVIEW_TYPE_SUBCONTRACT) {
						...
						// 本次评审为采购合同评审时取所有未分配采购合同(可能是之前销售合同评审也可能是销售+采购合同评审下货物)
