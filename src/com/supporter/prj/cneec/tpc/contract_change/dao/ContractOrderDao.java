...
@Repository
public class ContractOrderDao extends MainDaoSupport<ContractOrder, String> {

	/**
	 * 分页查询
	 */
	public List<ContractOrder> findPage(JqGrid jqGrid, ContractOrder contractOrder, String contractId, String authFilter) {
		if (contractId != null) {
			if(contractOrder != null) {
				// 列表页面搜索输入框可查询字段
				String keyword = contractOrder.getKeyword();
				...
				/* 以下是更多条件中选择项 */
				// 状态过滤
				if (contractOrder.getSwfStatus() != null) {
					jqGrid.addHqlFilter(" swfStatus = ? ", contractOrder.getSwfStatus());
				}
				//权限
	...
	public ContractOrder getOrderByChangeId(String changeId) {
		String hql = "from ContractOrder where changeId = ?";
		List<ContractOrder> sealList = this.find(hql, changeId);
	...
	public boolean checkContractChOrder(String changeId) {
		String hql = "from ContractOrder where changeId = ? and swfStatus=20";
		List<ContractOrder> sealList = find(hql, changeId);
	...
	public Integer getBusinessNoIndex(String NoCon) {
		Integer count = 1;
		String hql = "select max(t.businessNo) from " + ContractOrder.class.getName() + " t where t.businessNo like ?";
