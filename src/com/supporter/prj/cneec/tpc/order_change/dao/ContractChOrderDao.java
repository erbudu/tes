...
@Repository
public class ContractChOrderDao extends MainDaoSupport<ContractChOrder, String> {

	/**
	 * 分页查询
	 */
	public List<ContractChOrder> findPage(JqGrid jqGrid, ContractChOrder contractChOrder, String contractId, String authFilter) {
		if (contractId != null) {
			if(contractChOrder != null) {
				// 列表页面搜索输入框可查询字段
				String keyword = contractChOrder.getKeyword();
				...
				/* 以下是更多条件中选择项 */
				// 状态过滤
				if (contractChOrder.getSwfStatus() != null) {
					jqGrid.addHqlFilter(" swfStatus = ? ", contractChOrder.getSwfStatus());
				}
				//权限
	...
	public boolean checkContractChOrder(String changeId) {
		String hql = "from ContractChOrder where changeId = ? and swfStatus=20";
		List<ContractChOrder> sealList = find(hql, changeId);
	...
	public Integer getSerialNumberIndex(String NoHead) {
		Integer count = 1;
		String hql = "select max(t.serialNumber) from " + ContractChOrder.class.getName() + " t where t.serialNumber like ?";
	...
	public ContractChOrder getChByChangeId(String changeId) {
		String hql = "from ContractChOrder where changeId = ?";
		List<ContractChOrder> sealList = this.find(hql, changeId);
		if(sealList != null && sealList.size() > 0){
	...
	public Integer getBusinessNoIndex(String NoCon) {
		Integer count = 1;
		String hql = "select max(t.businessNo) from " + ContractChOrder.class.getName() + " t where t.businessNo like ?";
