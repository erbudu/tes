 ...
 */
@Repository
public class ContractEffectReportBfdDao extends MainDaoSupport<ContractEffectReportBfd, String> {
	 ...
	 * @return
	 */
	public List<ContractEffectReportBfd> findPage(JqGrid jqGrid, ContractEffectReportBfd contractEffectReportBfd) {
	...
	public boolean nameIsValid(String bfdId,String bfdName) {
		...
		if(StringUtils.isBlank(bfdId)) {//新建时
			hql = "from " + ContractEffectReportBfd.class.getName() + " where bfdName = ?";
			retList = this.retrieve(hql, bfdName);
		} else {//编辑时
			hql = "from " + ContractEffectReportBfd.class.getName() + " where bfdId != ? and bfdName = ?";
